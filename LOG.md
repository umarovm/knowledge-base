...
- [2026-07-21] session | Hermes: первый живой пилот Main. Реальная схема `~/.hermes/config.yaml` разошлась с предполагаемой (нет auxiliary/api_key, есть key_env + отдельный fallback_model) — SETUP.md 3.4 переписан. `main-combo` изначально собранный (groq/qwen3.6-27b→opencode-zen/big-pickle→gemini-2.5-flash→openrouter/tencent/hy3:free) весь отвалился (400/429×4/404/404); упрощён до одного Gemini AI Studio — заработало. TUI-баг «Setup Required» лечится командой `/model` внутри TUI. Troubleshooting-таблица дополнена.
- [2026-07-23] fix | Hermes agent полностью отменён (вместо него — Codex напрямую на удалённом VPS): удалена папка projects/hermes/ (SETUP.md + 4 заметки + index.md), тег `hermes` убран из словаря inbox/CLAUDE.md, индексы перегенерированы.
