...
**Открытые вопросы (решить при установке):**
- Механика ролей: GPT — менеджер/оркестратор, DeepSeek/Flash — исполнители/дебаггеры; сколько агентов и как передают друг другу таски — продумать при настройке.
- Как агент читает таскборд из KB (какая папка/файл, доступ) и как результат работы возвращается обратно (лог / BACKLOG.md).
- GitHub-токен для агента: сделать fine-grained PAT только на нужные репо (wikibot, трейдинг-боты), без доступа к остальным.
