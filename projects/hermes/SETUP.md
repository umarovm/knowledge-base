...
- **Финальная раскладка ролей (20.07):**
  ...
  - **Coder / Reviewer** (высокая цена ошибки) — идут через **OmniRoute**, как и остальные роли (кроме оркестратора, см. ниже); конкретные модели в комбо `combo-coder`/`combo-reviewer` выбираем позже при тестировании — ориентир: надёжный tool-calling, стартуем с DeepSeek V4. Kimi K2 — кандидат на эмпирический тест в течение 2-недельного пилота, не сразу в прод.
  ...
  - **Маршрутизация — уточнено (20.07):** все роли, кроме оркестратора, идут через **OmniRoute как основной путь**, с фолбэком на **прямой OpenRouter** — но это фолбэк уровня инфраструктуры (если сам процесс OmniRoute на VPS недоступен), а не про замену модели на другую. У оркестратора этот паттерн не действует: его основной путь (ChatGPT-подписка через Codex OAuth) существует только внутри OmniRoute, прямого эквивалента в OpenRouter нет — поэтому у него отдельный фолбэк (DeepSeek V4), тоже через OmniRoute (вторая ступень `combo-orchestrator`), а не переключение на прямой OpenRouter.
- **Репозитории (20.07):** wikibot и трейдинг-боты — отдельные git-репозитории, каждый клонируется на VPS отдельно, у каждого свой таскборд и свой git worktree под задачи. Fine-grained GitHub PAT покрывает оба репо, не весь аккаунт.
...
## 6. Дизайн воркфлоу: автоматизация кодинга (loop engineering)

По мотивам loop engineering (Addy Osmani, LangChain, тред sairahul1 про «не промптишь агента — строишь систему, которая промптит его сама»). Обязательные части лупа: триггер, память, разделение автор/проверяющий, условие остановки — без последнего луп крутится вечно и жрёт бюджет, пока ты спишь.

**Репозитории.** wikibot и трейдинг-боты — отдельные git-репозитории, каждый клонируется на VPS отдельно (`~/repos/wikibot`, `~/repos/трейдинг-боты`), у каждого свой таскборд (`projects/<имя>/BACKLOG.md` из KB, синхронизируется тем же способом, что описан в открытых вопросах шапки) и свой GitHub fine-grained PAT scope. Пилот (фаза 4 таблицы выше) — сначала один проект, второй подключаем через 2 недели, после обкатки.

**Триггер (heartbeat).** Ежедневный cron (утро, как в исходной идее) — оркестратор читает таскборд текущего проекта + вчерашние фейлы GitHub Actions, берёт следующую готовую задачу. Плюс `/background <задача>` из Telegram для разовых запросов вне расписания.

**Память.** Сам таскборд (что сделано / в работе / зафейлилось и почему) — не полагаемся, что агент помнит между запусками; вся история — в файле и в git-логе.

**Skills — написать до старта пилота.** По `SKILL.md` на каждый репозиторий: структура, как гонять тесты, деплой, конвенции, «почему не делаем так-то». Пишем тем же способом, что уже описан для скана вакансий (надиктовать голосом → отдать Hermes как основу скилла) — иначе агент каждый раз заново выясняет контекст с нуля.

**Изоляция (worktrees).** Каждая задача — свой `git worktree` на фиче-ветке внутри репозитория своего проекта; параллельные задачи (в т.ч. между wikibot и трейдинг-ботами) не сталкиваются файлами.
...
6. `goal_judge` (отдельная модель, не та, что писала код) решает «готово или нет»:
   ...
   - не готово после лимита попыток → эскалация тебе в Telegram, луп для этой задачи останавливается — не крутится вхолостую.

**Cost-cap.** Дневной потолок расходов (шире текущего алерта $5/мес) — если луп застрял в ретраях, не должен незаметно съесть бюджет за ночь.

**Самообучение.** Отдельный «hill-climbing»-луп строить не нужно — у Hermes это уже встроено (Curator сам сохраняет и улучшает skills из опыта); периодически (например, в конце недели пилота) смотреть, что он себе насохранял.

## 7. Бэклог-задача: скан вакансий (фаза 4)
...
## 8. OmniRoute — этап Б (фаза 5, не раньше)
...
Все роли, кроме оркестратора, идут через один и тот же паттерн: **основной путь — OmniRoute, инфраструктурный фолбэк — прямой OpenRouter** (на случай, если сам процесс OmniRoute на VPS недоступен — не про замену модели, а про недоступность гейтвея целиком).

```yaml
combo-coder:                   # модели внутри — выбрать позже при тестировании
  - openrouter/deepseek-v4     # стартовая точка: надёжный tool-calling

combo-reviewer:                 # ориентир тот же, что у coder — высокая цена ошибки
...
Для `delegate_task` по ролям coder/cleaner/reviewer модель передаётся явно на каждый вызов (см. открытый вопрос в шапке про named-профили — автопривязка ещё не гарантирована), плюс обязательно `workdir` на нужный git worktree в репозитории соответствующего проекта (wikibot и трейдинг-боты — репозитории отдельные):

- coder → `model: "combo-coder"`.
- reviewer → `model: "combo-reviewer"`.
- cleaner → `model: "combo-basechat"`.

`systemctl restart hermes` и проверь чат. Если сам OmniRoute недоступен (процесс упал) — фолбэк для coder/cleaner/reviewer: переключить `provider`/`base_url` обратно на прямой OpenRouter (`provider: "openrouter"`, соответствующая модель напрямую) — это стабильный откат уровня инфраструктуры. У оркестратора этот откат не работает так же: его фолбэк остаётся DeepSeek V4, но тоже через OmniRoute (вторая ступень `combo-orchestrator`), т.к. у ChatGPT-подписки нет прямого эквивалента в OpenRouter.

⚠️ Для cleaner free-провайдеры — основной путь (низкая цена ошибки); для coder/reviewer конкретные модели в комбо ещё не выбраны — стартуем с надёжного платного DeepSeek V4, донастраиваем по ходу пилота.
