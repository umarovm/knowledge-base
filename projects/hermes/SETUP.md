...
summary: "Проект разморожен (20.07.2026), эксперимент: автоматизация кодинга/дебага личных ботов (wikibot, трейдинг-боты) на изолированном VPS — таскборд из KB → код → PR → GitHub Actions review; агентный цикл, в отличие от трёх задач 17.07. Стек: OpenRouter (DeepSeek) + OmniRoute (free-тиры), без Claude Pro."
---

# Hermes Agent — SETUP (шапка = текущее состояние)

**Статус: разморожен, эксперимент (20.07.2026).** Причина: появилась задача, которая реально требует агентного цикла (в отличие от трёх задач 17.07 — все фиксированные pipeline): автоматизация кодинга для личных ботов (wikibot, трейдинг-боты) — агент читает таскборд из Knowledge Base, кодит, дебажит, пушит в GitHub, GitHub Actions проверяет PR перед мержем в main. Второй мотив: изолировать AI-кодинг от Mac (SSD страдает от нагрузки агентных инструментов вроде Codex).

Формат — эксперимент: сработает — оставляем и продолжаем, не понравится — VPS сносится.

**Механика ролей (решено 20.07, см. «Ключевые решения»):** оркестратор — ChatGPT-подписка через Codex OAuth в OmniRoute; coder/reviewer — DeepSeek V4 напрямую; cleaner и базовый чат Hermes — free-каскад OmniRoute с фолбэком на DeepSeek V4/Flash.

**Открыто, решить при установке:**
- Как агент читает таскборд из KB (какая папка/файл, доступ) и как результат работы возвращается обратно (лог / BACKLOG.md).
- GitHub-токен для агента: создать fine-grained PAT только на нужные репо (wikibot, трейдинг-боты), без доступа к остальным.
- Проверить на практике: делегированные саб-агенты (`delegate_task`) по умолчанию изолированы от файлов родителя — coder/cleaner саб-агентам нужно явно передавать `workdir` на склонированный репозиторий, иначе не смогут редактировать файлы.
- Автопривязка «роль → модель» через именованные профили — пока open feature request (#46880) в репо Hermes, не подтверждено как готовая фича. Поэтому модель для каждой роли задаём явно в каждом вызове `delegate_task`, а не полагаемся на ростер.

**Старые задачи (скан вакансий, отчёт о расходах, фриланс-воронка — фазы 4/6/7 ниже) — бэклог, не приоритет.** Текущий фокус — кодинг-автоматизация.
...
| 4 | **Задача (приоритет): автоматизация кодинга/дебага wikibot + трейдинг-боты** — таскборд из KB → код → тесты → PR → GitHub Actions review (2 недели обкатки на одном проекте) | ☐ |
| 5 | OmniRoute поверх (fallback + free-тиры) — только когда всё стабильно | ☐ |
| 5а | *(бэклог)* Задача: скан вакансий | ☐ |
| 6 | *(бэклог)* Задача: отчёт о расходах | ☐ |
| 7 | *(бэклог)* Задача: фриланс-воронка + UZ-networking + маппинг узбекских компаний/вилок/позиций | ☐ |
...
- Кодинг-задача (20.07): GitHub Actions обязателен как review-гейт перед мержем в main — агент не мержит напрямую.
- Фреймворк (20.07): **Hermes**, не OpenClaw. У Hermes готовый блок skills `software-development` (review-гейт перед коммитом, автономный PR-цикл branch→код→commit→push→PR, разбор фейлов GitHub Actions), нативный OpenRouter — совпадает с уже выбранным стеком. OpenClaw — более общий personal-assistant/message-router, под кодинг-ревью пришлось бы писать skills с нуля. Плюс инструкция ниже уже написана именно под Hermes.
- OmniRoute (20.07): используем не только для кодинг-задачи — общая инфраструктура для всех будущих задач на этом VPS (в т.ч. бэклог: скан вакансий, расходы), чтобы максимизировать бесплатные токены. Подтверждена из github.com/diegosouzapw/OmniRoute: локальный endpoint `:20128/v1`, 4-уровневый fallback (Subscription→API→Cheap→Free), 90+ провайдеров с free-тиром.
- **Финальная раскладка ролей (20.07):**
  - **Оркестратор** (декомпозиция задач, kanban, координация пайплайна) — ChatGPT-подписка через **Codex OAuth в OmniRoute** (`cx/gpt-5.5`, порт 1455) — официальная фича Tier-1 Subscription, тянет из уже оплаченной квоты ChatGPT Plus/Pro, без доп. API-затрат. Фолбэк — DeepSeek V4 (OpenRouter), если квота Codex исчерпана.
  - **Coder / Reviewer** (высокая цена ошибки) — DeepSeek V4 через OpenRouter напрямую, без free-подмены. Kimi K2 — кандидат на эмпирический тест в течение 2-недельного пилота, не сразу в прод.
  - **Cleaner** (линт/форматирование/commit-message/простые фиксы, низкая цена ошибки) и **базовый чат Hermes** (обычные вопросы в Telegram, не таск-пайплайн) — один и тот же free-каскад OmniRoute, стратегия `priority`: `if/kimi-k2` (безлимитно бесплатно, готовое комбо `free-forever`/`openclaw-free`) → `opencode-zen` → `kilo-gateway` → `mistral` (~1B токенов/мес) → `gemini` Flash-семейство → `groq` → `cerebras`; фолбэк последней ступенью — DeepSeek V4 Flash (OpenRouter, платно), когда все free-лимиты исчерпаны.
  - Провайдер `kiro` исключён из всех комбо — его ToS прямо запрещает использование через «OpenClaw и подобные инструменты-агентские обвязки» (та же категория, что Hermes).
  - Циркуит-брейкер/cooldown у OmniRoute встроены — сбойный или исчерпавший лимит провайдер пропускается сам, ручная retry-логика не нужна.
- **Где разворачивать OmniRoute (20.07): на VPS, не на Mac.** Hermes обращается к нему через `localhost:20128/v1` — должен быть на той же машине. Весь смысл переезда на VPS — изолировать AI-нагрузку от Mac (SSD), разворачивать OmniRoute обратно на Mac для Hermes бессмысленно. Порт наружу не открывается (см. чек-лист безопасности), доступ к дашборду — только через SSH-туннель. Если позже понадобится завести под free-tier роутинг ещё и локальные инструменты на Mac (Claude Code/Cursor) — не второй инстанс и не открытие порта, а встроенный **Remote mode** OmniRoute (`omniroute connect`/`tokens`, доступ по скоупнутым токенам).
...
Зачем: free-тиры (~1.6B бесплатных токенов/мес по каталогу провайдеров, включая безлимитный Kimi K2), ChatGPT-подписка для оркестратора, auto-fallback при недоступности DeepSeek, дашборд расходов, сжатие токенов (RTK+Caveman, 15–95%). **Разворачивается на VPS** (не на Mac) — Hermes ходит к нему по `localhost:20128`, и весь смысл VPS — снять AI-нагрузку с Mac.
...
### 7.1 Подключить провайдеров (Dashboard → Providers)

- **OpenRouter** — тот же ключ `sk-or-...` из 4.1 (даёт DeepSeek V4/Flash).
- **Codex (ChatGPT-подписка)** — Dashboard → Providers → Codex → добавить подключение → OAuth-логин своим ChatGPT Plus/Pro аккаунтом (порт 1455). Даёт `cx/gpt-5.5` и т.п. из уже оплаченной квоты подписки, без доп. API-затрат.
- **Free-провайдеры для cleaner/базового чата** (каждый добавляется в Providers, ключ не всегда нужен): `kimi` (безлимитно бесплатно через готовое комбо `free-forever`/`openclaw-free`, модель `if/kimi-k2`), `opencode-zen`, `kilo-gateway`, `mistral`, `gemini` (Flash-семейство), `groq`, `cerebras`. **Не подключай `kiro`** — его ToS прямо запрещает доступ через агентские обвязки вроде Hermes.

### 7.2 Собрать combo под роли

Три комбо со стратегией `priority` (drain-and-fallback):
...
combo-basechat:               # используется и для cleaner-задач, и для обычного чата Hermes
  ...
  - groq
  - cerebras
  - openrouter/deepseek-v4-flash   # фолбэк, платно, когда все free исчерпаны
```

Coder и reviewer роли **не заводятся отдельным combo** — там, где цена ошибки высокая, зовём `openrouter/deepseek-v4` напрямую (см. 4.1), без free-подмены.

Переключи Hermes на OmniRoute:
...
Для `delegate_task` по ролям coder/cleaner/reviewer модель передаётся явно на каждый вызов (см. открытый вопрос в шапке про named-профили — автопривязка ещё не гарантирована):

- coder / reviewer → `model: "openrouter/deepseek-v4"`, обязательно с `workdir` на склонированный репозиторий.
- cleaner → `model: "combo-basechat"`, тоже с `workdir` на репозиторий.

`systemctl restart hermes` и проверь чат. Если что-то глючит — верни `provider: "openrouter"`, `default: "deepseek/deepseek-v4"` — это твой стабильный откат.

⚠️ Free-модели — фолбэк и вспомогательные/cleaner-задачи, НЕ coder/reviewer: там нужен стабильный tool-calling, а free-провайдеры в этом плане менее предсказуемы.
...
- ☐ GitHub-доступ — fine-grained PAT, скоуп только на репо wikibot/трейдинг-боты (не на весь аккаунт/организацию), без прав на другие приватные репо
- ☐ Docker-контейнер агента не монтирует и не видит хостовые секреты — в `.env` контейнера только GitHub PAT + OpenRouter/OmniRoute ключи, никаких wallet-ключей и .env других ботов
- ☐ OmniRoute MITM/TPROXY (перехват TLS через кастомный CA для CLI, игнорирующих proxy env vars) — не включать по умолчанию; если понадобится для конкретного инструмента, ограничить только им и понимать, что ставится root CA в доверенные
