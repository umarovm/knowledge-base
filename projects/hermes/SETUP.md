...
| Роль | Модель | Фолбэк |
|---|---|---|
| **Main** (Telegram, роутинг между проектами, обычные задачи) | free-каскад `combo-basechat`: `if/kimi-k2` (безлимит) → groq → cerebras → opencode-zen → mimocode | последняя ступень — DeepSeek V4 Flash (платно), только если весь free-каскад исчерпан |
...
model:
  ...
  default: "combo-basechat"          # Main: Telegram, роутинг между проектами, обычные задачи — всегда free
  auxiliary:
    kanban_decomposer: { model: "combo-orchestrator" }   # = orchestration + goal_judge, GPT-5.6 Sol, платно
    compression:       { model: "combo-basechat" }
    title_generation:  { model: "combo-basechat" }
    web_extract:       { model: "combo-basechat" }
    # curator (skills-агент) НЕ удешевляем: GPT-5.6 Sol через combo-orchestrator, ошибки дорогие
```
...
- cleaner → `model: "combo-basechat"` (free-каскад, тот же что у Main)
