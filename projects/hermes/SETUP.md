...
summary: "Проект заморожен (17.07.2026): три плановые задачи не требуют agentic-фреймворка, дешевле как скрипты на существующем VPS; ad hoc-режим сам по себе не оправдывает отдельный VPS+Hermes. Инструкция ниже сохранена на случай, если появится задача, реально требующая агента."
---

# Hermes Agent — SETUP (шапка = текущее состояние)

**Статус: заморожен (17.07.2026).** Ничего не развёрнуто (фаза 0), дальше не двигаем. Причина: все три задачи (скан вакансий, анализ расходов, фриланс-воронка/UZ-мэппинг) — фиксированные пайплайны (fetch → один LLM-вызов → пуш), не требуют агентного цикла Hermes; дешевле и проще сделать скриптами на уже работающем VPS тем же стеком, что и остальные боты (Python + Telegram + Claude/OpenAI API + APScheduler + SQLite). Ad hoc-делегирование с телефона (`/background`) само по себе не оправдало отдельный VPS + framework.

**Разморозить, если появится:** конкретная задача, которая реально требует автономного многошагового agentic-цикла (динамический выбор инструментов, а не фиксированный pipeline) — тогда инструкция ниже актуальна как есть.
